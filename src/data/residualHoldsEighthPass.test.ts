...
interface OcrMidcutBan {
  ...
  seventhPassCorePinned: string[];
  eighthPassExpanded: string[];
  tableAxisPatterns: string[];
}

interface EighthPassFix {
  cardId: string;
  bookId: string;
  field: string;
  label: string;
...
const eighthPassDir = resolve(__dirname, "../../docs/verification/2026-09-12-residual-holds-eighth-pass");
...
const SEVENTH_PASS_PIN_FRAGMENTS = [
  "遺词用字",
  "辦士",
  "玛撤",
  "甘路",
...
describe("residual holds eighth-pass after seventh-pass inventory", () => {
  ...
  const ledger = loadJson<IsolationCmcLedger>(resolve(eighthPassDir, "isolation-cmc-ledger.json"));
  const ban = loadJson<OcrMidcutBan>(resolve(eighthPassDir, "ocr-midcut-ban.json"));
  const fixes = loadJson<EighthPassFix[]>(resolve(eighthPassDir, "eighth-pass-fixes.json"));
  const debt = loadJson<{ publicMix: Record<string, number> }>(resolve(eighthPassDir, "remaining-public-card-debt.json"));
  ...
  it("re-pins seventh-pass classes and bans the expanded eighth-pass leftovers across four fields", () => {
    for (const fragment of SEVENTH_PASS_PIN_FRAGMENTS) {
      expect(ban.seventhPassCorePinned.includes(fragment), fragment).toBe(true);
      expect(ban.seventhPassExpanded.includes(fragment) || ban.seventhPassCorePinned.includes(fragment), fragment).toBe(true);
  ...
  it("pins the eighth-pass fix inventory without changing card counts", () => {
    expect(fixes.length).toBeGreaterThan(80);
    expect(new Set(fixes.map((row) => row.cardId)).size).toBe(85);
    const labels = new Set(fixes.map((row) => row.label));
    for (const label of CORE_EIGHTH_PASS_LABELS) {
  ...
  it("keeps 66 volume audits on four fields and does not invent workbench v4 or Genesis image fixtures", () => {
    ...
    expect(existsSync(resolve(eighthPassDir, "full-vitest-blockers.md"))).toBe(true);
