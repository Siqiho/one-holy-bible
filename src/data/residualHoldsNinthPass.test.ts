...
interface OcrMidcutBan {
  ...
  eighthPassCorePinned: string[];
  ninthPassExpanded: string[];
  tableAxisPatterns: string[];
}

interface NinthPassFix {
  cardId: string;
  bookId: string;
  field: string;
  label: string;
...
const ninthPassDir = resolve(__dirname, "../../docs/verification/2026-09-12-residual-holds-ninth-pass");
...
const EIGHTH_PASS_PIN_FRAGMENTS = [
  "好诈",
  "尼布撤拉旦",
  "首先的 …未后的",
  "因力全人类",
...
describe("residual holds ninth-pass after eighth-pass inventory", () => {
  ...
  const ledger = loadJson<IsolationCmcLedger>(resolve(ninthPassDir, "isolation-cmc-ledger.json"));
  const ban = loadJson<OcrMidcutBan>(resolve(ninthPassDir, "ocr-midcut-ban.json"));
  const fixes = loadJson<NinthPassFix[]>(resolve(ninthPassDir, "ninth-pass-fixes.json"));
  const debt = loadJson<{ publicMix: Record<string, number> }>(resolve(ninthPassDir, "remaining-public-card-debt.json"));
  ...
  it("re-pins eighth-pass classes and bans the expanded ninth-pass leftovers across four fields", () => {
    for (const fragment of EIGHTH_PASS_PIN_FRAGMENTS) {
      expect(ban.eighthPassCorePinned.includes(fragment), fragment).toBe(true);
      expect(ban.eighthPassExpanded.includes(fragment) || ban.eighthPassCorePinned.includes(fragment), fragment).toBe(true);
    ...
    expect(fragments.length).toBeGreaterThan(230);
  ...
  it("pins the ninth-pass fix inventory without changing card counts", () => {
    expect(fixes.length).toBeGreaterThan(190);
    expect(new Set(fixes.map((row) => row.cardId)).size).toBe(71);
    const labels = new Set(fixes.map((row) => row.label));
    for (const label of CORE_NINTH_PASS_LABELS) {
  ...
  it("keeps 66 volume audits on four fields and does not invent workbench v4 or Genesis image fixtures", () => {
    ...
    expect(existsSync(resolve(ninthPassDir, "full-vitest-blockers.md"))).toBe(true);
