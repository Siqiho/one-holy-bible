...
不发明创世记 PNG、不发明 v4、不把隔离 CMC 写进公开包、不改 CI 去假装全量 `vitest` / GitHub `verify` 已绿。第十一轮对照面仍是公开数据包四字段和台账 JSONL。
