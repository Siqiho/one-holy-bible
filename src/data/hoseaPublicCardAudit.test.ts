...
describe("Hosea public card audit and isolation hold-queue", () => {
  const payload = loadBook("Hos");

  it("keeps the public Hosea package text-only, verse-mapped, and isolation-filtered", () => {
    expect(payload.bookId).toBe("Hos");
    expect(payload.textCards).toHaveLength(103);
    expect(payload.textCards.every((card) => card.type === "commentary" || card.type === "note")).toBe(true);

    const mix = { studyBible: 0, ocr: 0, cmc: 0, other: 0 };
    const verseIds = verseIdsFor("Hos");
    ...
    expect(mix).toEqual({ studyBible: 99, ocr: 4, cmc: 0, other: 0 });
  });

  it("does not republish confirmed mid-cut or garbage Hosea cards", () => {
