...
- 下列项目是第六轮之后仍可行动、但不能在本快照伪造原文的债务。结构化针见 `remaining-public-card-debt.json`。
...
隔离哈希 / 65 卷分册 / 哨兵与第五轮相同，见 `isolation-cmc-ledger.json`。
...
| `study-bible-acts-14-2-p050-n462` | `/23节；见本书9:2章` | 表头后正文是徒 18 亚波罗材料，删表头不能复原徒 14:2 |
| `study-bible-ps-41-13-p044-n198` | `其中首似乎不属于` | 「有些 / 三首」不唯一；本轮只改同卡 `完金` / `上常` / `剧歌` |

不是债务：`Iesous`（希腊转写）、`形成鲜明对比`、可读年表里的 `主后30`、`自己经历`（含 `己经` 子串）、`非利士地`、`希伯来话`、`隐罗结`、`ViaDolorosa`、`《和修》注`、`征收人头税`、`救赎大工` / `办理大工`、`着重`。

## 5. 交叉字段仍未对齐（不整段重写）

第六轮只按正文抄高置信残串：箴 12:18 summary 删 `13:19×［12节］`，创 11:26 summary/searchText 删庙塔图注。雅各 2:5 body「拣选术语」vs summary「选举术语」两词都合法，不改。其余 summary 分叉多为括号切边 / 省略号截断。
