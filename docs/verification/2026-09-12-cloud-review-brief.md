...
- 简报日期：2026-09-12
- 性质：**只读审阅**。本文件只汇总已落盘的卷级 / 残差报告，不发明研修本原文，不新开第十轮 OCR。
- 残差栈顶端：`cursor/ohb-residual-holds-ninth-pass-94c2`（PR #39）
- 顶端 SHA：`f936d2c682b8e1f71c2d8e5f910cb2f1360460a8`（`f936d2c`）
- 远端扫描：`git fetch origin --prune` 后**没有** `tenth` / `eleventh` 及以上残差分支；第九轮即当前 tip。
- 公开包钉数（第二轮起至第九轮不变）：66 卷、文字卡 **10,096** = 研修本 **9,089** + CMC **858** + OCR **146** + 信息系列 **3**。
- 隔离 ∩ 公开：`[]`（合同 keep）。
- 本简报分支：`cursor/cloud-review-brief-ohb`。只加这份文档，**不改** `public/data/books/*.json`。
...
| 9（tip） | [#39](https://github.com/Siqiho/one-holy-bible/pull/39) | `cursor/ohb-residual-holds-ninth-pass-94c2` | 71 | 同上 |

结构化 mix 针：`docs/verification/2026-09-12-residual-holds-second-pass/cross-pack.json` 与第九轮 `remaining-public-card-debt.json` 的 `publicMix` 一致。

第九轮钉过的校验（`docs/verification/2026-09-12-residual-holds-ninth-pass/validate.txt`）：

- 公开审计 + 残差测试：78 files / 312 tests passed
- `generatePublicBibleData --validate-only`：66 卷、31,102 CUV/KJV、10,096 文字卡、0 unsafe
- `validatePublicRepository`：通过

各轮只改仍留包内的 `study-bible-*` 四字段（`title` / `body` / `summary` / `searchText`），不改经文，不改 OCR 卡正文。字段级清单在对应 `*-pass-fixes.json`。审阅时读报告与禁表即可，不必重跑 OCR。

---

## c. 隔离 ∩ 公开 = ∅

合同从第一过贯穿到第九轮 tip，测试与台账都钉空集。
...
抽样口径不变：隔离 CMC 是无解释经文残句，有解释才 keep。本快照不新抬、不改六源映射。第九轮测试 `src/data/residualHoldsNinthPass.test.ts` 重钉哈希、65 卷分册、哨兵与公开不相交。

---

## d. 剩余 hold / OCR hold-as-class 债务

下列项目**仍留在第九轮之后的债务面**。无 Mac `Resources/`、无源 PDF、无卷级 OCR 台账（约翰除外）则不能复原，**禁止补造原文**。完整针：`docs/verification/2026-09-12-residual-holds-ninth-pass/remaining-public-card-debt.md` 与同目录 `.json`。
...
第九轮 `remaining-public-card-debt.json` 的 `publicMidCutStillInPackage` 共 23 条（hold-in-place）：
...
上表 23 条均标注 hold-in-place（含嵌套引号 `1chr-intro`，改一处不等于复原内层）。不是债务的合法子串（`dia加人名属格`、`堕落人性`、`出人意料`、`浩瀚宇宙`、`征收人头税` 等）见第九轮 `keepNotDebt`，勿当禁串回扫。

交叉字段：第九轮已按正文 / searchText 闭合路 11:8「情词迫场」与雅 5:4「残人自肥 / 收劃」。雅 2:5「拣选 / 选举」两词合法，不改。其余 summary 分叉多为括号切边 / 省略号，不整段重写。
...
明细：`docs/verification/2026-09-12-residual-holds-ninth-pass/full-vitest-blockers.md`。原则：记录真实缺失，不发明创世记图像 / 工作台 v4 / 假隔离资源，不改 CI 装绿。

GitHub `verify` 作业会跑 `validate:public-data`（应绿）再跑全量 `npm test`（仍红）。失败面**不读**第九轮公开包修补，与 PR #32–#38 同一类缺口。
...
## f. 如何在云端审阅、不把栈合进本地 Mac

残差史是 **#32←#33←…←#39** 开放栈，底在 PR #31，**都还没进 `main`**。本简报另开分支，就是为了不在 tip 上再叠一轮 OCR、也不逼 Mac 快进合并。

### 不要做

- 不要 `git pull` / merge 任一 `cursor/ohb-residual-holds-*-pass-*` 或 1:1 hold-queue 分支到本地 `main`。
- 不要在 Mac 上对公开包做第十轮 OCR，或按用户全库数「补齐」启导本 / 故事卡 / 隔离 CMC。
- 不要为了让 GitHub `verify` 全绿而补造创世记 PNG、工作台 v4、或改 CI 跳过 `npm test`。
- 不要发明研修本后文去救 347 张 keep-hold 或 22 条中截。

### 推荐做法（云端只读）

1. **只开这一份简报 PR**（`cursor/cloud-review-brief-ohb`，draft）。Files changed 应只有 `docs/verification/2026-09-12-cloud-review-brief.md`。
2. **残差正文仍以 PR #39 为准**。GitHub 上打开 [PR #39](https://github.com/Siqiho/one-holy-bible/pull/39)，用 *Files changed* / *Commits* 看第九轮公开包修补与台账；需要更早轮次时沿 base 链点 #38 → #32，不要 checkout 到笔记本。
3. **要跑命令就开 Cursor Cloud Agent**，检出 `cursor/ohb-residual-holds-ninth-pass-94c2`（或本简报分支，公开包与 tip 相同）。本环境跑身份：<https://cursor.com/agents/bc-2caecb2e-bb43-5915-9faf-25ea3f51366b>。
4. 云端只跑上一节的公开包校验 + 残差测试。全量 `npm test` 红是**已知资产缺口**，不是第九轮回归失败。
5. 对照顺序建议：本简报 → 第九轮 `report.md` / `remaining-public-card-debt.md` / `full-vitest-blockers.md` → 需要时再打开对应卷的 `*-1to1-hold-queue/report.md`。
6. Mac 本地继续停在你原来的工作树（约翰卡 / `main` / Resources）。审阅结论写在 GitHub review 或云端 follow-up，等你明确要求再谈合栈或恢复 v4。

### 顶端只读指纹（2026-09-12 checkout）
...
bc2d8e7 review: third-pass residual OCR bans and isolation CMC ledger
3fcdb1d docs: pin residual-holds second-pass validation counts
d9ec262 review: second-pass residual holds and public OCR hold-as-class
19ba9da docs: pin Titus/2 Thessalonians/Leviticus/2 Peter validation counts

$ git status
On branch cursor/ohb-residual-holds-ninth-pass-94c2
nothing to commit, working tree clean

$ git rev-parse HEAD
f936d2c682b8e1f71c2d8e5f910cb2f1360460a8
```

检出本简报分支后，`git status` 只会多这份文档；公开包与第九轮 tip 字节级相同。
