...
- 下列项目是第八轮之后仍可行动、但不能在本快照伪造原文的债务。结构化针见 `remaining-public-card-debt.json`。
...
隔离哈希 / 65 卷分册 / 哨兵与第七轮相同，见 `isolation-cmc-ledger.json`。
...
| `study-bible-1cor-11-4-p019-n164` | `就必须先了当时的社会背景` | 第七轮只改同卡 `头中`；第八轮只改同卡 gloss closer；`先了` 不唯一 |
| `study-bible-1cor-11-29-p021-n117` | `吃喝自己的吃喝就是定自己的罪了）` | 第七轮只改同卡 `亵读` / `響告`；缺开括号不补造 |
...
不是债务：`Iesous`、`形成鲜明对比`、可读年表里的 `主后30`、`自己经历`（含 `己经` 子串）、`非利士地`、`希伯来话`、`隐罗结`、`ViaDolorosa`、`《和修》注`、`征收人头税` / `纳人头税`、`悦纳人`、`救赎大工` / `办理大工`、`着重`、`善事上常用`、`昆虫` / `昆兰` / `撤离`、`墻垣` / `墻壁`、年表/大纲脚注 `年*` / `2:12*`、`美好计划`、`味道很不好细心栽培`。

## 5. 交叉字段仍未对齐（不整段重写）

第八轮只按正文抄高置信残串：林前 14:14 body 已是「人的"灵"」，summary/searchText 的 `人的*灵""` 按正文闭合。雅各 2:5 body「拣选术语」vs summary「选举术语」两词都合法，不改。其余 summary 分叉多为括号切边 / 省略号截断。
