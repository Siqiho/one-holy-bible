...
interface OcrMidcutBan {
  ...
  fifthPassExpanded: string[];
  tableAxisPatterns: string[];
}

interface FifthPassFix {
  cardId: string;
  bookId: string;
  field: string;
  label: string;
...
const fifthPassDir = resolve(__dirname, "../../docs/verification/2026-09-12-residual-holds-fifth-pass");
...
const FOURTH_PASS_PIN_FRAGMENTS = [
  "迎百农",
  "迎拿",
  "舍已",
  "人侵",
...
describe("residual holds fifth-pass after fourth-pass inventory", () => {
  ...
  const ledger = loadJson<IsolationCmcLedger>(resolve(fifthPassDir, "isolation-cmc-ledger.json"));
  const ban = loadJson<OcrMidcutBan>(resolve(fifthPassDir, "ocr-midcut-ban.json"));
  const fixes = loadJson<FifthPassFix[]>(resolve(fifthPassDir, "fifth-pass-fixes.json"));
  const debt = loadJson<{ publicMix: Record<string, number> }>(resolve(fifthPassDir, "remaining-public-card-debt.json"));
  ...
  it("re-pins fourth-pass classes and bans the expanded fifth-pass leftovers across four fields", () => {
    for (const fragment of FOURTH_PASS_PIN_FRAGMENTS) {
      expect(ban.fourthPassStillBanned.includes(fragment), fragment).toBe(true);
    ...
    const fragments = [
      ...
      ...ban.fifthPassExpanded,
    ];
    expect(fragments.length).toBeGreaterThan(140);
  ...
  it("pins the fifth-pass fix inventory without changing card counts", () => {
    expect(fixes.length).toBeGreaterThan(280);
    expect(new Set(fixes.map((row) => row.cardId)).size).toBe(99);
    const labels = new Set(fixes.map((row) => row.label));
    for (const label of CORE_FIFTH_PASS_LABELS) {
  ...
  it("keeps 66 volume audits on four fields and does not invent workbench v4 or Genesis image fixtures", () => {
    ...
    expect(existsSync(resolve(fifthPassDir, "full-vitest-blockers.md"))).toBe(true);
