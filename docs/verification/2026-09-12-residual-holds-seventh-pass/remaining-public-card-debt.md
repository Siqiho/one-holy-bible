...
| `study-bible-1cor-7-39-p015-n079` | `意*愿意` | 星号落在词中，不是唯一开/闭引号 |
| `study-bible-acts-3-18-p037-n312` | `主将得救的11:18*` | 年表倾倒，不补造 |
| `study-bible-acts-4-13-p015-n084` | `4:31（*放胆》` | 中截 / 书名号倾倒 |
| `study-bible-john-19-14-p048-n463` | `约"是：*` | 中截，不补造 |

不是债务：`Iesous`、`形成鲜明对比`、可读年表里的 `主后30`、`自己经历`（含 `己经` 子串）、`非利士地`、`希伯来话`、`隐罗结`、`ViaDolorosa`、`《和修》注`、`征收人头税`、`救赎大工` / `办理大工`、`着重`、`善事上常用`、`昆虫` / `昆兰` / `撤离`、`墻垣` / `墻壁`、年表/大纲脚注 `年*` / `2:12*`。

## 5. 交叉字段仍未对齐（不整段重写）

第七轮只按正文抄高置信残串：提前 6:5 summary/searchText 的 `遺责` 按 body `谴责` 对齐；提前 4:10 searchText 的 `"万人*` 按 body `"万人"` 闭合。雅各 2:5 body「拣选术语」vs summary「选举术语」两词都合法，不改。其余 summary 分叉多为括号切边 / 省略号截断。
