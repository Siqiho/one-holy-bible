...
interface OcrMidcutBan {
  ...
  fifthPassCorePinned: string[];
  tableAxisPatterns: string[];
}

interface SixthPassFix {
  cardId: string;
  bookId: string;
  field: string;
  label: string;
...
const sixthPassDir = resolve(__dirname, "../../docs/verification/2026-09-12-residual-holds-sixth-pass");
...
const FIFTH_PASS_PIN_FRAGMENTS = [
  "介人",
  "宜告",
  "藉者",
  "官廷",
...
describe("residual holds sixth-pass after fifth-pass inventory", () => {
  ...
  const ledger = loadJson<IsolationCmcLedger>(resolve(sixthPassDir, "isolation-cmc-ledger.json"));
  const ban = loadJson<OcrMidcutBan>(resolve(sixthPassDir, "ocr-midcut-ban.json"));
  const fixes = loadJson<SixthPassFix[]>(resolve(sixthPassDir, "sixth-pass-fixes.json"));
  const debt = loadJson<{ publicMix: Record<string, number> }>(resolve(sixthPassDir, "remaining-public-card-debt.json"));
  ...
  it("re-pins fifth-pass classes and bans the expanded sixth-pass leftovers across four fields", () => {
    for (const fragment of FIFTH_PASS_PIN_FRAGMENTS) {
      expect(ban.fifthPassCorePinned.includes(fragment), fragment).toBe(true);
      expect(ban.fifthPassStillBanned.includes(fragment) || fragment === "，，", fragment).toBe(true);
    ...
    expect(fragments.length).toBeGreaterThan(160);
  ...
  it("pins the sixth-pass fix inventory without changing card counts", () => {
    expect(fixes.length).toBeGreaterThan(50);
    expect(new Set(fixes.map((row) => row.cardId)).size).toBe(20);
    const labels = new Set(fixes.map((row) => row.label));
    for (const label of CORE_SIXTH_PASS_LABELS) {
  ...
  it("keeps 66 volume audits on four fields and does not invent workbench v4 or Genesis image fixtures", () => {
    ...
    expect(existsSync(resolve(sixthPassDir, "full-vitest-blockers.md"))).toBe(true);
