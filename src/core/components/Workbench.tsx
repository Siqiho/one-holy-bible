...
export const layoutStorageKey = "one-holy-bible-public-layout";
export const paperThemeStorageKey = "one-holy-bible-public-paper-theme";
...
export const readingFontStorageKey = "one-holy-bible-public-reading-font";
...
const resourceEditsStorageKey = "one-holy-bible-public-resource-edits";
const deletedResourceIdsStorageKey = "one-holy-bible-public-deleted-resource-ids";
export const commentaryLibrariesStorageKey = "one-holy-bible-public-commentary-libraries";
...
const WorkbenchReadOnlyContext = createContext(true);
...
export function ResourceCard({
  ...
  const readOnly = useContext(WorkbenchReadOnlyContext);
  ...
  const canUnsyncResource = !readOnly && Boolean(resource.debugMeta?.sourceWorkbenchCardId && onUnsyncResource);
  const canDeleteResource = !readOnly && Boolean(showDeleteAction && onDeleteResource);
...
export function Workbench({
  ...
  const [resourceEdits, setResourceEdits] = useState<ResourceEditDrafts>(() => readOnly ? {} : storedResourceEdits());
  const [deletedResourceIds, setDeletedResourceIds] = useState<Set<string>>(() => readOnly ? new Set<string>() : storedDeletedResourceIds());
  ...
  const visibleResources = useMemo(() => (
    readOnly ? navigationValidatedResources : filterDeletedResources(editableResources(navigationValidatedResources, resourceEdits), deletedResourceIds)
  ), [readOnly, deletedResourceIds, navigationValidatedResources, resourceEdits]);
  ...
  useEffect(() => {
    // Unloaded book resources must not be treated as deleted in the public reader.
    if (onRequestBook) return;
  ...
  }, [onRequestBook, visibleResources]);
  ...
  function editResourceCopy(resourceId: string, draft: ResourceTextDraft): ResourceEditResult {
    if (readOnly) return { persisted: false };
  ...
  async function updateWorkbenchResourceCopy(resourceId: string, draft: ResourceTextDraft) {
    if (readOnly || !onUpdateWorkbenchResource || updateInFlightResourceIdRef.current) return;
  ...
  async function unsyncResource(resourceId: string) {
    if (readOnly || !onUnsyncResource || unsyncInFlightResourceIdRef.current) return;
    const resource = visibleResources.find((item) => item.id === resourceId);
  ...
  return (
    <WorkbenchReadOnlyContext.Provider value={readOnly}>
    ...
    </WorkbenchReadOnlyContext.Provider>
  );
}
