...
interface OcrMidcutBan {
  ...
  fourteenthPassExpanded: string[];
  fourteenthPassCorePinned: string[];
  tableAxisPatterns: string[];
}

interface FourteenthPassFix {
  cardId: string;
  bookId: string;
  field: string;
  label: string;
...
const fourteenthPassDir = resolve(__dirname, "../../docs/verification/2026-09-12-residual-holds-fourteenth-pass");
...
const CORE_FOURTEENTH_PASS_LABELS = [
  "宜称→宣称",
  "宜讲→宣讲",
  "宜布→宣布",
  "宜道→宣道",
...
const THIRTEENTH_PASS_PIN_FRAGMENTS = [
  "排尼基",
  "后餅",
  "干万人",
  "干代，",
...
describe("residual holds fourteenth-pass after thirteenth-pass inventory", () => {
  ...
  const ledger = loadJson<IsolationCmcLedger>(resolve(fourteenthPassDir, "isolation-cmc-ledger.json"));
  const ban = loadJson<OcrMidcutBan>(resolve(fourteenthPassDir, "ocr-midcut-ban.json"));
  const fixes = loadJson<FourteenthPassFix[]>(resolve(fourteenthPassDir, "fourteenth-pass-fixes.json"));
  const debt = loadJson<{ publicMix: Record<string, number> }>(resolve(fourteenthPassDir, "remaining-public-card-debt.json"));
  ...
  it("re-pins thirteenth-pass cores and bans the expanded fourteenth-pass leftovers across four fields", () => {
    for (const fragment of THIRTEENTH_PASS_PIN_FRAGMENTS) {
      expect(ban.thirteenthPassCorePinned.includes(fragment) || ban.thirteenthPassExpanded.includes(fragment), fragment).toBe(true);
      expect(studyBlob.includes(fragment), fragment).toBe(false);
    }
    for (const fragment of ban.fourteenthPassCorePinned) {
      expect(ban.fourteenthPassExpanded.includes(fragment), fragment).toBe(true);
      expect(studyBlob.includes(fragment), fragment).toBe(false);
    }
    const fragments = [
      ...
      ...ban.fourteenthPassExpanded,
    ];
    expect(fragments.length).toBeGreaterThan(350);
    ...
    expect(/5:37。6:46/.test(studyBlob)).toBe(false);
  });
  ...
  it("pins the fourteenth-pass fix inventory without changing card counts", () => {
    expect(fixes.length).toBeGreaterThan(80);
    expect(new Set(fixes.map((row) => row.cardId)).size).toBe(31);
    const labels = new Set(fixes.map((row) => row.label));
    for (const label of CORE_FOURTEENTH_PASS_LABELS) {
      expect(labels.has(label), label).toBe(true);
    }
    const uniqueOcr = new Set(
      fixes
        .filter((row) => !["hexiu-missing-paren", "dirty-hexiu-closer", "dirty-period-see", "verse-list-period"].includes(row.label))
        .map((row) => row.cardId),
    );
    const hexiuCards = new Set(fixes.filter((row) => row.label === "hexiu-missing-paren").map((row) => row.cardId));
    const dirtyHexiu = new Set(fixes.filter((row) => row.label === "dirty-hexiu-closer").map((row) => row.cardId));
    const punctCards = new Set(
      fixes.filter((row) => ["dirty-period-see", "verse-list-period"].includes(row.label)).map((row) => row.cardId),
    );
    expect(uniqueOcr.size).toBe(12);
    expect(hexiuCards.size).toBe(9);
    expect(dirtyHexiu.size).toBe(7);
    expect(punctCards.size).toBe(3);
  ...
  it("keeps 66 volume audits on four fields and does not invent workbench v4 or Genesis image fixtures", () => {
    ...
    for (const name of auditTests) {
      ...
      if (text.includes("宜称") && text.includes("好儿里") && text.includes("膜责") && text.includes("l0Culi")) {
        fourteenthPinned += 1;
      }
    }
    expect(fourField).toBe(66);
    expect(fourteenthPinned).toBe(66);
    expect(existsSync(resolve(fourteenthPassDir, "full-vitest-blockers.md"))).toBe(true);
