...
- 下列项目是第四轮之后仍可行动、但不能在本快照伪造原文的债务。结构化针见 `remaining-public-card-debt.json`。
...
隔离哈希 / 65 卷分册 / 哨兵与第三轮相同，见 `isolation-cmc-ledger.json`。
...
不是债务：`Iesous`（希腊转写）、`形成鲜明对比`、可读年表里的 `主后30`、`自己经历`（含 `己经` 子串）。
