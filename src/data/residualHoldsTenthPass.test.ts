...
const tenthPassDir = resolve(__dirname, "../../docs/verification/2026-09-12-residual-holds-tenth-pass");
...
const NINTH_PASS_PIN_FRAGMENTS = [
  "约瀚",
  "希伯来¥经",
  "《创，世记》",
  "（*离散；diaspora）",
...
describe("residual holds tenth-pass after ninth-pass inventory", () => {
  ...
  const ledger = loadJson<IsolationCmcLedger>(resolve(tenthPassDir, "isolation-cmc-ledger.json"));
  const ban = loadJson<OcrMidcutBan>(resolve(tenthPassDir, "ocr-midcut-ban.json"));
  const fixes = loadJson<TenthPassFix[]>(resolve(tenthPassDir, "tenth-pass-fixes.json"));
  const debt = loadJson<{ publicMix: Record<string, number> }>(resolve(tenthPassDir, "remaining-public-card-debt.json"));
  ...
  it("re-pins ninth-pass classes and bans the expanded tenth-pass leftovers across four fields", () => {
    for (const fragment of NINTH_PASS_PIN_FRAGMENTS) {
      expect(ban.ninthPassCorePinned.includes(fragment), fragment).toBe(true);
      expect(ban.ninthPassExpanded.includes(fragment) || ban.ninthPassCorePinned.includes(fragment), fragment).toBe(true);
    ...
    expect(fragments.length).toBeGreaterThan(250);
  ...
  it("pins the tenth-pass fix inventory without changing card counts", () => {
    expect(fixes.length).toBeGreaterThan(210);
    expect(new Set(fixes.map((row) => row.cardId)).size).toBe(150);
    const labels = new Set(fixes.map((row) => row.label));
    for (const label of CORE_TENTH_PASS_LABELS) {
  ...
  it("keeps 66 volume audits on four fields and does not invent workbench v4 or Genesis image fixtures", () => {
    ...
    expect(existsSync(resolve(tenthPassDir, "full-vitest-blockers.md"))).toBe(true);
