...
describe("Zephaniah public card audit and isolation hold-queue", () => {
  const payload = loadBook("Zeph");

  it("keeps the public Zephaniah package text-only, verse-mapped, and isolation-filtered", () => {
    expect(payload.bookId).toBe("Zeph");
    expect(payload.textCards).toHaveLength(56);
    expect(payload.textCards.every((card) => card.type === "commentary" || card.type === "note")).toBe(true);

    const mix = { studyBible: 0, ocr: 0, cmc: 0, other: 0 };
    const verseIds = verseIdsFor("Zeph");
  ...
  it("does not republish confirmed mid-cut or garbage Zephaniah cards", () => {
