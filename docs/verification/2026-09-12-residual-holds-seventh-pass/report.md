...
- 结论：隔离 ∩ 公开仍为 ∅。第七轮不新抬隔离 CMC，不改六源标识，不假设 Mac `Resources/`。公开研修本 **9,089**、公开文字卡 **10,096** 不变。前五轮已禁残串继续为空；第七轮又从公开研修本四字段清掉 42 张仍留包内的高置信 OCR / 闭合引号 / 表头数字 / 交叉字段脏拷（`遺词` / `辦士` / `玛撤` / `撤但` / `甘路` / `牙方` / `亵读` / `好淫` / `《1约》` / `《I约》` / `五干人` / `上帝右边*` / `为1"我主"` / `"奥秘事*` / `被称为1"儿子"` 等），并把第六轮核心禁串（`仂敌` / `大工家` / `完金` / `上常` / `剧歌` / `人门` / `意人必须` / 日期一字线 / 表尾 / 混标点）再钉进第七轮禁表。
...
## 3. 已 fix（公开研修本 42 张，不撤卡）

只改 `study-bible-*` 的 `title` / `body` / `summary` / `searchText`。不改经文，不改 OCR 卡。字段级记录见 `seventh-pass-fixes.json`（124 行）。
...
| `二万三干` / `五干人` | 三千 / 五千 | 林前 10:8；徒 4:4（同卡已有 3,000 / 10.000） |
| `《1约》` / `《I约》` / `披肩或头中` | 旧约 / 旧约 / 头巾 | 林前 10:18；诗 71:22（与《新约》对举）；林前 11:4 |
| `上帝右边*` / `"奥秘事*` / `"一次*` / `"与上帝同行*` 等 | 闭合引号 | 诗 110:1；林前 4:1；来 1:2 / 11:5；徒 9:23 / 10:39；路 1:32 / 6:20；太 24:30 / 24:34；尼 9:38；箴 11:30；诗 4:3 / 16:11 / 37:9 / 77:20；罗 1:5；多 1:1；提前 4:10 searchText；王下 8:11 |
| `为1"我主"` / `被称为1"儿子"` | 删表头数字 | 诗 110:1；来 2:10 |
| `原文直译是*为自己的名"` / `他们的*父亲"` | 星号当开引号 | 徒 15:14；伯 1:6 |
| `受咒诅的"'` | 去掉多余单引号 | 徒 10:39 |
