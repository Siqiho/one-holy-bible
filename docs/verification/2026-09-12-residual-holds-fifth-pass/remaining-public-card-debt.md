...
- 下列项目是第五轮之后仍可行动、但不能在本快照伪造原文的债务。结构化针见 `remaining-public-card-debt.json`。
...
隔离哈希 / 65 卷分册 / 哨兵与第四轮相同，见 `isolation-cmc-ledger.json`。
...
不是债务：`Iesous`（希腊转写）、`形成鲜明对比`、可读年表里的 `主后30`、`自己经历`（含 `己经` 子串）、`非利士地`、`希伯来话`、`隐罗结`、`ViaDolorosa`、`《和修》注`、`征收人头税`。

## 5. 交叉字段仍未对齐（不整段重写）

第五轮只按正文抄高置信残串：雅各 2:13 `引人14`→`引入14`，箴 22:29 summary `贏得`→`赢得`。雅各 2:5 body「拣选术语」vs summary「选举术语」两词都合法，不改。其余 summary 分叉多为括号切边 / 省略号截断。
